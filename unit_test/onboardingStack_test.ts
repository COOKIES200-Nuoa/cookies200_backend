...
describe('OnboardingStack', () => {
    ...
    test('Lambda role has necessary permissions', () => {
        ...
        const receivedStatements  = policyResource.Properties.PolicyDocument.Statement;

        // Assert the entire policy statements array (with the exact expected structure)
        expect(receivedStatements).toEqual(expect.arrayContaining([
            {
                Action: [
            ...
            },
            {
                ...
                Effect: 'Allow',
                Resource: 'arn:aws:cognito-identity:ap-southeast-1:891377270638:identitypool/*'
            }
        ]));
    });
});